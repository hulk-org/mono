...
description: "A kura collection at `kura-org/private/universal/kura-spaces/collections/command-line-adventures/` records substrate command-line edge cases (ARG_MAX, FD limits, lock contention, quoted-path normalization, etc.) with diagnosis + fix + lesson per entry. Path uses `kura-spaces/` (not `kura/`); two entries as of 2026-06-05 (ARG_MAX + Xcode-source-control lock pileup)."
metadata:
  node_type: memory
  type: reference
...
**Path:** `private/universal/substrate/collectives/kura-org/private/universal/kura-spaces/collections/command-line-adventures/`

**Note on path:** the collection lives under `kura-spaces/collections/`, NOT `kura/collections/`. An earlier version of this memory had the wrong path; verified against on-disk state 2026-06-05.
...
**2026-06-05 entry added:** Xcode source-control polling piles up `git status --untracked-files=all` invocations on substrate-scale trees, serializing all git-write operations behind the index.lock. Two entries now in the collection; pattern of "substrate-scale + standard tool = pathological resource pressure" is recurring.
