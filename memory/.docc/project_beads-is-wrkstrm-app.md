...
description: gastownhall/beads is the task management backbone for wrkstrm apps — task graphs, dependencies, gates, agent-native concurrency via Dolt; Koma executes, Beads orchestrates
type: project
---
...
Ghost (interprets, projects) → Koma (how — execution). Don't reinvent task/dependency
systems in wrkstrm apps — use Beads as the backbone. Koma pipelines execute individual
Beads steps. The Ghost bridges the two: reads a ready Beads issue, projects a Koma
...
Discovered 2026-04-20 during Swarm vs Koma investigation.
